// penilai-satu-atasan-langsung.component.ts
...
@Component({
  selector: 'app-penilai-satu-atasan-langsung',
  templateUrl: './penilai-satu-atasan-langsung.component.html',
  styleUrls: ['./penilai-satu-atasan-langsung.component.scss']
})
export class PenilaiSatuAtasanLangsungComponent implements OnInit, OnDestroy {
  // ===== DATATABLE =====
  ...
  private loadLeaderStaffData(periodId: string, orgId: string): void {
    ...
    this.tupTpaService.viewLeaderStaffData(periodId, orgId).subscribe({
      ...
      error: (err) => {
        console.error('API ERROR getViewLeaderStaffData:', err);
        this.dataTable = [];
        this.originalRows = [];
        this.loadingTable = false;
      }
  ...
  public onDetail(row: any): void {
    ...
    this.router.navigate(
      ['/penilai-satu-atasan-langsung/daftar-data-penilaian-pegawai', assessorId, periodId],
