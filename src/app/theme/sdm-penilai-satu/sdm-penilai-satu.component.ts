...
@Component({
  selector: 'app-sdm-penilai-satu',
  templateUrl: './sdm-penilai-satu.component.html',
  styleUrls: ['./sdm-penilai-satu.component.scss']
})
export class SdmPenilaiSatuComponent implements OnInit, OnDestroy {
  ...
  /** Template bindings (filter controls). */
  public isStrukturDisabled = false;
  ...
  /** Datatable config (pagination + info aktif, search bawaan dimatikan) */
  ...
  /** Ambil angka dari objek API (beberapa nama field per lingkungan). */
  private pickNum(row: any, keys: string[]): number {
    if (!row) {
      return 0;
    }
  ...
  private mapLeaderStaffRows(apiRows: any[], selectedPeriodeLabel?: string): any[] {
    ...
    return (apiRows || []).map((x: any) => ({
      ...
      // API viewLeaderStaffData: total_staff, total_assessment, total_assessment_done
      jumlahPegawai: this.pickNum(x, ['total_staff', 'jumlah_staff', 'totalStaff', 'jumlah_pegawai']),
      jumlahPenilaian: this.pickNum(x, ['total_assessment', 'jml_assessment', 'totalAssessment', 'jumlah_penilaian']),
      penilaianSelesai: this.pickNum(x, ['total_assessment_done', 'jml_assessment_selesai', 'totalAssessmentDone', 'penilaian_selesai']),
  ...
  onPeriodeChange(val: any): void {
    ...
    if (val != null && val !== '') {
      this.loadLeaderStaffData(String(val), '1480');
    }
  }
  ...
  applyFilter(): void {
    const periodId = this.selectedPeriode != null && this.selectedPeriode !== ''
      ? String(this.selectedPeriode)
      : '1';
    this.loadLeaderStaffData(periodId, '1480');
    this.successToast('Filter diterapkan.');
  }
  ...
  public onDetail(row: any): void {
    ...
    this.router.navigate(
      ['/sdm-penilai-satu/daftar-data-penilaian-pegawai', assessorId, periodId],
